...
ZIP_PATH="$DIST_DIR/liangqin-brand-design-openclaw-${PACKAGE_DATE}.zip"
INSTALLER_PATH="$DIST_DIR/liangqin-brand-design-installer-${PACKAGE_DATE}.sh"
...
cat > "$INSTALLER_PATH" <<EOF2
#!/bin/sh
set -eu

SKILLS_ROOT="\${SKILLS_ROOT:-\$HOME/.openclaw/skills}"
WORKSPACE_SKILL_DEST="\${WORKSPACE_SKILL_DEST:-\$HOME/.openclaw/workspace/skills/liangqin-brand-design}"
RUN_TEST=1
TEST_MESSAGE=""
...
ZIP_FILE="\$TMP_DIR/liangqin-brand-design.zip"
...
SKILL_SOURCE="\$EXTRACT_DIR/liangqin-brand-design"
SKILL_DEST="\$SKILLS_ROOT/liangqin-brand-design"

mkdir -p "\$SKILLS_ROOT"
rm -rf "\$SKILL_DEST"
cp -R "\$SKILL_SOURCE" "\$SKILL_DEST"

python3 "\$SKILL_DEST/scripts/publish_skill.py" \
  --source "\$SKILL_DEST" \
  --dest "\$WORKSPACE_SKILL_DEST" \
  --workspace-dest ''
