...
在进入任何高保真输出前，先按下面顺序判断：

1. **Context Gate**
   - 品牌 brief、产品事实、参考材料、真实 logo / 图像 / 图标资产、目标保真度是否足够。
   - 若不足，优先输出 structured spec、placeholder 或待补资料清单，不直接伪造成品。
2. **Surface Gate**
   - 先识别这次交付是网页、H5、报价卡、设计画布、演示稿、可交互原型、动效还是独立 HTML。
   - 不允许把所有承载都做成网页落地页。
3. **Variation Gate**
   - 只有用户明确要探索，或任务确实需要多方向比较时，才补读 `design-packs/`。
   - `design-packs/` 是受控 tweak，不是第二套品牌系统。
4. **Material Gate**
   - logo、图像、图标、报价、产品参数和案例事实不足时，优先降级，不用假数据硬凑高保真。
5. **Runtime Gate**
   - 需要真实打开、缩放、滚动、打印、导出或转发的交付物，必须遵守对应 `artifact-surfaces/*.json` 的运行合同。
6. **Verification Gate**
   - 交付前至少检查结构、运行、品牌拟合和最终格式。
7. **Handoff Gate**
   - 明确当前结果是 spec、视觉草稿、原型、最终成品还是阻断状态。
...
4. 再跑 Context Gate 与 Surface Gate，先判断这次请求属于页面蓝图、报价卡、H5、画布、原型还是其它视觉交付
5. 页面类请求只从白名单模块库中选模块；视觉类请求先定义信息主次、构图、图像角色与图标降级策略
6. 只有需要探索时才读取 `design-packs/`，并把它们视为受控 tweak 轴
7. 再判断这次变化属于“内容变化”“承载变化”还是“渠道变化”，三者都不得被理解成风格切换
8. 按品牌边界约束结构、语气、CTA、中文表达和图标工艺
9. 先确认 `protocols/brand-assets.zh-CN.json` 中的真实 logo 资产已可读；若不可读，停止在结构化 spec
10. 用 `EXECUTION-CHECKLIST.md` 做生成前 / 生成中 / 生成后自检
11. 如需落到具体渠道，优先调整信息密度、画幅、交付形式，不切换品牌真源
12. OpenClaw 默认按 Tier 3 消费标准包；如果上下文不足，至少保住 Tier 1 或 Tier 2 的边界
13. 如与品牌边界冲突，回退到 Foundation DNA 与 DESIGN.md；如图标或复杂细节无法稳定成立，优先降级而不是自由发挥
...
- 不在上下文不足时，编造参数、图像、案例事实或假数据把版面填满
- 不把 deck、报价卡、H5、设计画布或动效统一套成网页落地页
